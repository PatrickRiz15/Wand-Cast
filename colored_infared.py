...
class WandProcessor:
    def __init__(self, frame_width, frame_height):
        ...
        self.color_tracing_frame = cv2.cvtColor(self.tracing_frame, cv2.COLOR_GRAY2BGR)
    ...
    def process_frame(self, frame):
        ...
        if self.color_tracing_frame.shape != color_frame.shape:
            self.color_tracing_frame = cv2.cvtColor(np.zeros_like(frame), cv2.COLOR_GRAY2BGR)
        ...
        # keypoints = self.detector.detect(bg_subtracted_threshold)
        ...
        #Erase the trace if after 5 seconds
        self.current_time = datetime.now()
        if self.last_keypoint_time:
            time_diff = (self.current_time - self.last_keypoint_time).total_seconds()
            if time_diff > 5:
                self.color_tracing_frame = np.zeros_like(self.color_tracing_frame)
                self.keypoints.clear()
                self.last_keypoint_time = None
        
        # Draw the keypoints on the original frame
        for kp in keypoints:
            cv2.circle(color_frame, tuple(map(int, calculate_keypoint_center(kp))), 5, (0, 0, 255), -1)

        # Combine the trace with the original frame
        combined_frame = cv2.add(color_frame, self.color_tracing_frame)
    ...
    def update_trace(self, keypoints):
        ...
        if keypoints:
            ...
            if self.last_keypoint_time:
                time_diff = (self.current_time - self.last_keypoint_time).total_seconds()
                if time_diff < 0.5:
                    # pt1 = tuple(map(int, self.keypoints[-1].pt))
                    # pt2 = tuple(map(int, keypoints[0].pt))
                    ...
                    if not (abs(pt1[0] - pt2[0]) > self.max_trace_distance or abs(pt1[1] - pt2[1]) > self.max_trace_distance):
                        cv2.line(self.color_tracing_frame, pt1, pt2, (255, 255, 0), 2)
    ...
    try:
        while True:
            ...
            if kinect.has_new_infrared_frame():
                ...
                # Add trace on top of the infrared frame
                combined_frame = processor.process_frame(processed_infrared_frame)
